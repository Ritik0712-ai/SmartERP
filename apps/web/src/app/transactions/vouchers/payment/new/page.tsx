...
export default function PaymentVoucherPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      api.get('/suppliers?companyId=' + activeCompanyId),
    ]).then(([lr, pr]) => {
      if (lr.data.success) setLedgers(lr.data.data.ledgers);
      if (pr.data.success) setParties(pr.data.data.suppliers);
    });
  }, [activeCompanyId]);
  ...
  return (
    <AppShell>
      <div className="space-y-6">
        <PageHeader title="Payment Voucher" description="Record money paid to a supplier" backHref="/transactions/vouchers" />
        ...
        <form onSubmit={onSubmit} className="space-y-6">
          <Card>
            ...
            <CardContent className="grid grid-cols-1 gap-4 md:grid-cols-3">
              ...
              <div>
                <Label htmlFor="v-party">Supplier *</Label>
