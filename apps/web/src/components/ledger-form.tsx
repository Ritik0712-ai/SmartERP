...
export function LedgerForm({ mode, ledgerId }: { mode: 'create' | 'edit'; ledgerId?: string }) {
  ...
  useEffect(() => {
    ...
    api.get<ApiSuccess<{ groups: any[] }>>(`/ledger-groups?companyId=${activeCompanyId}`)
      .then((r) => { if (r.data.success) setGroups(r.data.data.groups); });
  }, [activeCompanyId]);

  useEffect(() => {
    if (mode === 'edit' && ledgerId && activeCompanyId) {
      api.get<ApiSuccess<any>>(`/ledgers/${ledgerId}?companyId=${activeCompanyId}`)
        .then((res) => {
          if (res.data.success) {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = mode === 'create'
        ? await api.post<ApiSuccess<any>>(`/ledgers?companyId=${activeCompanyId}`, payload)
        : await api.patch<ApiSuccess<any>>(`/ledgers/${ledgerId}?companyId=${activeCompanyId}`, payload);
