...
function SupplierForm({ mode, supplierId }: { mode: 'create' | 'edit'; supplierId?: string }) {
  ...
  useEffect(() => {
    if (mode === 'edit' && supplierId && activeCompanyId) {
  ...
  }, [mode, supplierId, activeCompanyId]);
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = mode === 'create'
        ? await api.post<ApiSuccess<any> | ApiError>(`/suppliers?companyId=${activeCompanyId}`, payload)
        : await api.patch<ApiSuccess<any> | ApiError>(`/suppliers/${supplierId}?companyId=${activeCompanyId}`, payload);
    ...
    <AppShell>
      <div className="space-y-6">
        <PageHeader
          ...
          description={mode === 'create' ? 'Add a new supplier — a Sundry Creditors ledger is auto-created' : 'Update supplier details'}
          backHref="/suppliers"
        />
        {serverError && (
            ...
            <Card>
              ...
              <CardContent className="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <div>
                  ...
                  <Input id="gstNumber" value={form.gstNumber} onChange={(e) => update('gstNumber', e.target.value)} />
                </div>
                <div>
                  <Label htmlFor="panNumber">PAN</Label>
                  <Input id="panNumber" value={form.panNumber} onChange={(e) => update('panNumber', e.target.value)} />
                </div>
              </CardContent>
            </Card>
            <Card>
...
export default function NewSupplierPage() {
  return <ProtectedRoute><SupplierForm mode="create" /></ProtectedRoute>;
}
