  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      const payload: any = {
        ...
        openingQuantity: parseFloat(form.openingQuantity) || 0,
      ...
      const res = await api.post<ApiSuccess<any>>(`/stock-items?companyId=${activeCompanyId}`, payload);
  ...
  return (
    <AppShell>
      <div className="space-y-6">
        <PageHeader title="New Stock Item" description="Add a product or material to your inventory" backHref="/inventory/stock-items" />
          ...
          <Card>
            ...
            <CardContent className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div className="md:col-span-2">
                ...
                <Input value={form.name} onChange={e => update('name', e.target.value)} placeholder="USB Cable Type-C" required />
              </div>
              <div>
                <Label>SKU * (unique)</Label>
                <Input value={form.sku} onChange={e => update('sku', e.target.value)} placeholder="USB-C-001" required />
              </div>
              <div>
                <Label>HSN Code</Label>
                <Input value={form.hsnCode} onChange={e => update('hsnCode', e.target.value)} placeholder="8544" />
              </div>
              <div>
              ...
              <div className="md:col-span-2">
                <Label>Barcode</Label>
                <Input value={form.barcode} onChange={e => update('barcode', e.target.value)} placeholder="1234567890123" />
              </div>
              <div className="md:col-span-2">
                <Label>Description</Label>
                <Input value={form.description} onChange={e => update('description', e.target.value)} placeholder="High quality braided cable" />
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div>
                ...
                <Input type="number" min="0" step="1" value={form.openingQuantity} onChange={e => update('openingQuantity', e.target.value)} />
              </div>
              <div>
                <Label>Reorder level (low stock alert)</Label>
                <Input type="number" min="0" step="1" value={form.reorderLevel} onChange={e => update('reorderLevel', e.target.value)} placeholder="10" />
              </div>
            </CardContent>
          </Card>
