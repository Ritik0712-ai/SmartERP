 ...
 */
import { PrismaClient, VoucherTypeCode, UserRole, GroupType } from '@prisma/client';
...
async function seedVoucherTypes() {
  const types = [
    { code: VoucherTypeCode.PURCHASE, name: 'Purchase Voucher', description: 'Records purchases from suppliers' },
    { code: VoucherTypeCode.SALES, name: 'Sales Voucher', description: 'Records sales to customers' },
    { code: VoucherTypeCode.RECEIPT, name: 'Receipt Voucher', description: 'Records incoming payments' },
    { code: VoucherTypeCode.PAYMENT, name: 'Payment Voucher', description: 'Records outgoing payments' },
    { code: VoucherTypeCode.JOURNAL, name: 'Journal Voucher', description: 'Accounting adjustments' },
    { code: VoucherTypeCode.CONTRA, name: 'Contra Voucher', description: 'Cash ↔ Bank transfers' },
    { code: VoucherTypeCode.CREDIT_NOTE, name: 'Credit Note', description: 'Sales return' },
    { code: VoucherTypeCode.DEBIT_NOTE, name: 'Debit Note', description: 'Purchase return' },
  ];
...
async function seedAdminAndDemo() {
  ...
  if (!existingCompany) {
    ...
    await prisma.userCompanyRole.create({
      data: { userId: admin.id, companyId: company.id, role: UserRole.ADMIN },
    });
    ...
    const ledgerGroups = [
      { name: 'Assets', groupType: GroupType.ASSET },
      { name: 'Liabilities', groupType: GroupType.LIABILITY },
      { name: 'Income', groupType: GroupType.INCOME },
      { name: 'Expenses', groupType: GroupType.EXPENSE },
      { name: 'Capital', groupType: GroupType.LIABILITY },
      { name: 'Sundry Debtors', groupType: GroupType.ASSET, parent: 'Assets' },
      { name: 'Sundry Creditors', groupType: GroupType.LIABILITY, parent: 'Liabilities' },
      { name: 'Sales Account', groupType: GroupType.INCOME, parent: 'Income' },
      { name: 'Purchase Account', groupType: GroupType.EXPENSE, parent: 'Expenses' },
      { name: 'Cash', groupType: GroupType.ASSET, parent: 'Assets' },
      { name: 'Bank', groupType: GroupType.ASSET, parent: 'Assets' },
      { name: 'Duties & Taxes', groupType: GroupType.LIABILITY, parent: 'Liabilities' },
    ];
