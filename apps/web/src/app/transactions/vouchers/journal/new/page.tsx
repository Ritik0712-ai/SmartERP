...
export default function JournalVoucherPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (pr.data.success) setParties(pr.data.data.ledgers);
    });
  }, [activeCompanyId]);
  ...
  return (
    <AppShell>
      <div className="space-y-6">
        <PageHeader title="Journal Voucher" description="Pure accounting adjustments between ledgers" backHref="/transactions/vouchers" />
        ...
        <form onSubmit={onSubmit} className="space-y-6">
          <Card>
            ...
            <CardContent className="grid grid-cols-1 gap-4 md:grid-cols-3">
              ...
              <div>
                <Label htmlFor="v-party">Reference Ledger *</Label>
                <select id="v-party" value={partyLedgerId} onChange={e => setPartyLedgerId(e.target.value)} required className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm">
                  <option value="">— Select reference ledger —</option>
